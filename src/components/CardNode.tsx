...
export function CardNode({ data }: { data: Card & { isAlert: boolean, onClick: () => void } }) {
  ...
  return (
    <div 
      className={`card flex flex-col p-4 w-52 cursor-pointer sway-node hover:!animation-none hover:scale-105 hover:-shift-y-1 shift-anim duration-300 bg-[#1E222D] ${
        data.isAlert 
    ...
    >
      ...
      <div className="flex items-center gap-2 text-[#E0E0E6] mb-1">
        <span className="font-mono text-sm ls-wide">Card Number {data.last4}</span>
      </div>
