...
export function CardNode({ data }: { data: Card & { isAlert: boolean, onClick: () => void } }) {
  return (
    <div 
      className={`card flex flex-col p-4 w-52 cursor-pointer hover:border-blue-500/50 transition-colors shadow-2xl bg-[#1E222D] ${
    ...
    >
      <div className="flex justify-between items-start mb-4">
        ...
        {data.isAlert ? (
          <span className="px-1.5 py-0.5 bg-red-500/20 text-red-400 text-[9px] rounded font-bold uppercase">Limit Breach</span>
        ) : (
      ...
      <div className="flex items-center gap-2 text-[#E0E0E6] mb-1">
        <span className="font-mono text-sm tracking-wider">•••• •••• •••• {data.last4}</span>
      </div>

      {data.limit && (
        <div className="mt-4 pt-4 border-t border-white/5 flex justify-between items-end">
          <span className={`text-xs ${data.isAlert ? 'text-red-400' : 'text-white/40'}`}>Limit: ${data.limit.toLocaleString()}/mo</span>
        </div>
      )}
